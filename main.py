...
def convert_sheet(df_input: pd.DataFrame, transaction: str):
    ...
    for row in df_input.itertuples():
        ...
        try:
            output.loc[len(output)] = [row._3[:11],
                                       qty * int(row.Quantity),
                                       row.Price,
                                       round(int(row.Price)/qty, 2)
                                       ]
...
if __name__ == "__main__":
    ...
    for result in pathlib.Path(".").iterdir():
        if "transaction" in (result.name).lower() and "output" not in (result.name).lower():
